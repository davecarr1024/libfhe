...
class Window(Aspect):
    ...
    def initGraphics(self):
        ...
        pygame.display.set_caption(self.getVar("title","page"))
    ...
    def msg_update(self, time, dtime):
        ...
        if time - self.lastFrameTime > self.frameTime:
            ...
            self.entity.publish("render3")
            self.set2dProjection()
            self.entity.publish("render2")
        ...
        for event in pygame.event.get():
            ...
            self.tryCall("input_%s" % eventName,None, event)
                
    def input_quit(self, event):
        self.log("window quit")
        self.entity.getRoot().publish('shutdown')
    ...
    def input_keyDown(self, event):
        if event.key == pygame.K_ESCAPE:
            ...
            self.entity.publish('keyDown', name = name, mods = mods, key = event.key)

    def input_keyUp(self, event):
        name, mods = self.keyInfo(event)
        self.entity.publish('keyUp', name = name, mods = mods, key = event.key)

    def input_joyButtonDown(self, event):
        self.entity.publish('joyButtonDown',joy = event.joy, button = event.button)

    def input_joyButtonUp(self, event):
        self.entity.publish('joyButtonUp',joy = event.joy, button = event.button)

    def input_joyAxisMotion(self, event):
        self.entity.publish('joyAxisMotion',joy = event.joy, value = event.value, axis = event.axis)

    def input_mouseButtonDown(self, event):
        buttonName, pos, absPos = self.mouseInfo(event)
        hitObjects = self.doPicking(pos)
        self.entity.publish('mouseButtonDown',button = event.button, pos = pos, absPos = absPos, buttonName = buttonName, hitObjects = hitObjects)

    def input_mouseButtonUp(self, event):
        buttonName, pos, absPos = self.mouseInfo(event)
        hitObjects = self.doPicking(pos)
        self.entity.publish('mouseButtonUp', button = event.button, pos = pos, absPos = absPos, buttonName = buttonName, hitObjects = hitObjects)
    ...
    def input_mouseMotion(self, event):
        ...
        self.entity.publish('mouseMotion', pos = pos, absPos = absPos, buttons = event.buttons, buttonNames = buttonNames)
