...
class Object:
    ...
    def __init__(self, name = None, parent = None, vars = None, children = None):
        ...
        name = name or self.__class__.__module__
    ...
    def addChildren(self, data):
        for name, data in data.iteritems():
            ...
            data["name"] = name
            util.dynload(childType)(parent = self,**data)
    ...
    def isVarType(self, v):
        varTypes = [bool,int,float,str,Vec2,Vec3,Quat]
        if any([isinstance(v,t) for t in varTypes]):
            return True
        elif isinstance(v,list) or isinstance(v,tuple):
            return v and all(map(self.isVarType,v))
        elif isinstance(v,dict):
            return v and all(map(self.isVarType,v.values()))
        else:
            return False
    ...
    def getVars(self):
        ...
        for key, val in self.__dict__.iteritems():
            if key != 'name' and not key.startswith('_') and self.isVarType(val):
                vars[key] = val
        return vars
    ...
    def isDescendant(self, obj):
        return obj.getPath().startswith(self.getPath())

    def searchChildren(self, cond):
        for obj in self.children.itervalues():
            if cond(obj):
                return obj

    def filterChildren(self, cond):
        return filter(cond,self.children.values())
