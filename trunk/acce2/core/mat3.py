...
if __name__ == "__main__":
    ...
    assert equal(v,Mat3.identity().invert() * v)
    assert equal(v - t, Mat3.translation(t).invert() * v)
    assert equal(v / s, Mat3.scale(s).invert() * v)
    assert equal(v.rotate(-r),Mat3.rotation(r).invert() * v)
