...
def prof(outputFile):
    execString = 'python -m cProfile -o ' + outputFile + ' Main.py ' + ' '.join(sys.argv[1:])
    os.system(execString)
    
def getList(stats, *amount):
    w, l = stats.get_print_list(amount)
    return l

def getStats(stats, function):
    recursiveCalls, ncalls, tottime, cumtime, callers = stats.stats[function]
    return ncalls, tottime, tottime/ncalls , cumtime, callers

def formatFunctionName(functionName):
    file, line, name = functionName
    if(file == "~"):
        func = name
    else:
        func = file + ":" + `line` + "(" + name + ")"
...
def fN(number):
    return str(round(number,3)).rjust(11)

def fN2(number):
    return str(number).rjust(11)
    
def printLine(line):
    print "   " + fN(line[0]) + " |" + fN(line[1]) + " |" + fN2(line[2]) + " |" + fN(line[3])+ "|   " + line[4]

def procStats(outputFile):
    data = []
    
    p = pstats.Stats(outputFile)
    #p.strip_dirs().sort_stats('time').print_stats(10).print_callers(10)
    
    totalTime = p.total_tt
    p = p.strip_dirs().sort_stats('time')
    l = getList(p, 25)
    
    for f in l:
        ncalls, tottime, percall, cumtime, callers = getStats(p, f)
        data.append([tottime, tottime/totalTime, ncalls, percall, formatFunctionName(f)])
    
    print "   Total Time  | % of Run   | # of Calls | Per Call  | function"
    print "   ____________________________________________________________"
    for l in data:
        printLine(l)
        
    file = open("profilerOutput/profileGraph.txt", 'w')
    for i, l in enumerate(data):
        file.write(str(i+1) + " " + l[4] + " " + str(l[1]) + "\n")
    file.close()
    ...
    for i, l in enumerate(data):
        if(i < len(data)-1):
            ...
            ytics += '"' + l[4] + '" ' + str(i+1)
    ytics += ')'
    plotCommand = 'plot "profilerOutput/profileGraph.txt" using 3:1 with fstep'
    plot(ytics)
    plot(plotCommand)
        
outputFile = 'profilerOutput/profile.dat'
prof(outputFile)
procStats(outputFile)
